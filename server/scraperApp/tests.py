...
class CourseTestCase(TestCase):
    ...
    def test_course_results(self):
        ...
        response = self.client.get('http://server:8000/api/courses/')
        self.assertEqual(response.status_code, 200)

    def test_course_results_search(self):
        """search fonctionalities"""
        response = self.client.get('http://server:8000/api/courses/', {'search': 'Management'})
        for result in response.data['results']:
            self.assertIn('Management', result['name'])

    def test_course_results_filter(self):
        """fitler fonctionalities"""
        response = self.client.get('http://server:8000/api/courses/', {'cities': ['CITY_1']})
        self.assertEqual(len(response.data['results']), 1)
        for result in response.data['results']:
            self.assertIn('CITY_1', result['information']['city'])    

        response = self.client.get('http://server:8000/api/courses/', {'cities': 'CITY_1', 'portals': 'Test Portal 1'})
        self.assertEqual(len(response.data['results']), 1)
        ...
        response = self.client.get('http://server:8000/api/courses/', {'cities': 'CITY_1', 'portals': 'Test Portal 2'})
        self.assertEqual(len(response.data['results']), 0)

    def test_course_pagination(self):
        """ Pagination """
        response = self.client.get('http://server:8000/api/courses/', {'limit': 2})
...
class BookmarkTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        resp = cls.api_client.post('http://server:8000/api/jwt/create/', {'email': cls.user.email, 'password': '12345678'}, format='json')
        cls.token = resp.data['access']
                
    def test_bookmark_creation(self):
        """Create bookmark"""
        response = self.api_client.post('http://server:8000/api/bookmarks/', {'user': self.user.id, 'course': self.course_3.id}, format='json')
        self.assertEqual(response.status_code, 401)
        self.api_client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.api_client.post('http://server:8000/api/bookmarks/', {'user': self.user.id, 'course': self.course_1.id}, format='json')
    ...
    def test_bookmarks_listing(self):
        ...
        self.api_client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.api_client.get('http://server:8000/api/bookmarks/', {'page': 2, 'limit': 1}, format='json')
        self.assertEqual(response.status_code, 200)        
...
class UserTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        user.is_active = True
        user.save()
        response = cls.api_client.post('http://server:8000/api/jwt/create/', cls.current_user , format='json')
        cls.token = response.data['access']

    def test_user_registration(self):
        """user register"""
        response = self.api_client.post('http://server:8000/api/users/', self.user_register, format='json')
        self.assertEqual(response.status_code, 201)
    
    def test_user_login(self):
        """user login"""
        response = self.api_client.post('http://server:8000/api/jwt/create/', self.user_login, format='json')
        self.assertEqual(response.status_code, 401)
    ...
    def test_current_user(self):
        """user me"""
        self.api_client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.api_client.get('http://server:8000/api/users/me/', format='json')
        self.assertEqual(response.status_code, 200)